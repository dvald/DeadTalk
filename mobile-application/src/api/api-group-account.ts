...
import type { UsernameChangeRequest, EmailChangeRequest, PasswordChangeRequest, TFAGenResponse, TFASetupRequest, TFARemoveRequest, SessionListItem, ChangeLocaleBody, DeleteAccountRequest } from "./definitions";
...
export class ApiAccount {
    ...
    public static ChangeUsername(body: UsernameChangeRequest): RequestParams<void, ChangeUsernameErrorHandler> {
        return {
            method: "POST",
            url: getApiUrl(`/account/username` + generateURIQuery({ _time: Date.now() })),
            json: body,
    ...
    public static ChangeEmail(body: EmailChangeRequest): RequestParams<void, ChangeEmailErrorHandler> {
        return {
            method: "POST",
            url: getApiUrl(`/account/email` + generateURIQuery({ _time: Date.now() })),
            json: body,
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(400, "INVALID_TFA_CODE", handler.badRequestInvalidTfaCode)
    ...
    public static ChangePassword(body: PasswordChangeRequest): RequestParams<void, ChangePasswordErrorHandler> {
        return {
            method: "POST",
            url: getApiUrl(`/account/password` + generateURIQuery({ _time: Date.now() })),
            json: body,
            handleError: (err, handler) => {
                new RequestErrorHandler()
    ...
    public static GenerateTFA(): RequestParams<TFAGenResponse, CommonAuthenticatedErrorHandler> {
        return {
            method: "GET",
            url: getApiUrl(`/account/tfa/generate` + generateURIQuery({ _time: Date.now() })),
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(401, "*", handler.unauthorized)
                    .add(500, "*", "serverError" in handler ? handler.serverError : handler.temporalError)
    ...
    public static SetupTFA(body: TFASetupRequest): RequestParams<void, SetupTFAErrorHandler> {
        return {
            method: "POST",
            url: getApiUrl(`/account/tfa/setup` + generateURIQuery({ _time: Date.now() })),
            json: body,
            handleError: (err, handler) => {
                new RequestErrorHandler()
    ...
    public static RemoveTFA(body: TFARemoveRequest): RequestParams<void, RemoveTFAErrorHandler> {
        return {
            method: "POST",
            url: getApiUrl(`/account/tfa/disable` + generateURIQuery({ _time: Date.now() })),
            json: body,
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(400, "INVALID", handler.badRequestInvalid)
    ...
    public static GetSessions(): RequestParams<SessionListItem[], CommonAuthenticatedErrorHandler> {
        return {
            method: "GET",
            url: getApiUrl(`/account/sessions` + generateURIQuery({ _time: Date.now() })),
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(401, "*", handler.unauthorized)
                    .add(500, "*", "serverError" in handler ? handler.serverError : handler.temporalError)
    ...
    public static CloseSession(id: string): RequestParams<void, CommonAuthenticatedErrorHandler> {
        return {
            method: "DELETE",
            url: getApiUrl(`/account/sessions/${encodeURIComponent(id)}` + generateURIQuery({ _time: Date.now() })),
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(401, "*", handler.unauthorized)
                    .add(500, "*", "serverError" in handler ? handler.serverError : handler.temporalError)
    ...
    public static CloseAllSessions(): RequestParams<void, CommonAuthenticatedErrorHandler> {
        return {
            method: "POST",
            url: getApiUrl(`/account/sessions/close` + generateURIQuery({ _time: Date.now() })),
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(401, "*", handler.unauthorized)
                    .add(500, "*", "serverError" in handler ? handler.serverError : handler.temporalError)
    ...
    public static ChangeLocale(body: ChangeLocaleBody): RequestParams<void, CommonAuthenticatedErrorHandler> {
        return {
            method: "POST",
            url: getApiUrl(`/account/locale` + generateURIQuery({ _time: Date.now() })),
            json: body,
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(401, "*", handler.unauthorized)
    ...
    public static DeleteAccount(body: DeleteAccountRequest): RequestParams<void, DeleteAccountErrorHandler> {
        return {
            method: "POST",
            url: getApiUrl(`/account/delete` + generateURIQuery({ _time: Date.now() })),
            json: body,
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(400, "INVALID_TFA_CODE", handler.badRequestInvalidTfaCode)
