...
import type { UserProfile, UpdateProfileBody, UploadProfileImageResponse } from "./definitions";
...
export class ApiProfile {
    ...
    public static GetProfile(username: string): RequestParams<UserProfile, GetProfileErrorHandler> {
        return {
            method: "GET",
            url: getApiUrl(`/profile/${encodeURIComponent(username)}` + generateURIQuery({ _time: Date.now() })),
    ...
    public static UpdateProfile(body: UpdateProfileBody): RequestParams<void, UpdateProfileErrorHandler> {
        return {
            method: "POST",
            url: getApiUrl(`/profile` + generateURIQuery({ _time: Date.now() })),
            json: body,
            handleError: (err, handler) => {
                new RequestErrorHandler()
    ...
    public static UpdateImage(formParams: UpdateImageFormParameters): RequestParams<UploadProfileImageResponse, UpdateImageErrorHandler> {
        ...
        if (formParams.image !== undefined) {
            appendFileToForm(formData, "image", formParams.image);
        }

        return {
            method: "POST",
            url: getApiUrl(`/profile/image` + generateURIQuery({ _time: Date.now() })),
            form: formData,
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(400, "*", handler.badRequest)
