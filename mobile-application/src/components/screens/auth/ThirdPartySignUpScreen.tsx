...
export default function ThirdPartySignUpScreen({ navigation }: { navigation: NavigationProp<any> }) {
    ...
    const usesStaticEmail = !!(route.params as any)["email"];

    const [email, setEmail] = useState((route.params as any)["email"] || "");
    const [username, setUsername] = useState((route.params as any)["username"] || "");
    ...
    const doSignUp = () => {
        ...
        Request.Do(ApiAuth.ThirdPartyRegister({ id, token, username, password, locale: language, email }))
            .onSuccess(response => {
                setBusy(false);
