...
const getMissingTranslations = (lines) => {
    const res = [];
    let started = false;
...
const main = () => {
    ...
    const locales = findTranslationConfigFiles().map((l) => {
        ...
        return {
            id: lang,
            file: l,
            missingFile: Path.resolve(__dirname, "..", "i18n", "locales", lang + ".missing.txt"),
        };
    });
    ...
    for (let locale of locales) {
        ...
        for (let key of Object.keys(locales)) {
            if (!locales[key]) {
                ...
                if (preparedMissingTranslations.length === 0) {
                    console.log(`Error: The locale file has more missing translations than provided.`);
                    error = true;
                    break;
                }
        ...
        if (preparedMissingTranslations.length > 0) {
            console.log(`Error: The locale file has less missing translations than provided.`);
            continue;
        }
