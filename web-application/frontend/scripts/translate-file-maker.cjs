...
const generateTranslationConfigurationFile = (resources) => {
...
const getResourcesFromTranslationsFile = (file) => {
    const locale = JSON.parse(FS.readFileSync(file).toString());
    return locale;
};

const searchTranslationUsages = (file) => {
...
const mergeResources = (resource1, resource2) => {
    const result = {};
...
const mergeResourcesFirstSide = (resource1, resource2) => {
    const result = {};
...
const findTranslationConfigFiles = () => {
    const files = FS.readdirSync(Path.resolve(__dirname, "..", "i18n", "locales"));
    const result = [];
    for (let file of files) {
        if (file.endsWith(".json")) {
            result.push(Path.resolve(__dirname, "..", "i18n", "locales", file));
        }
    }
    return result;
};

const scanDirectories = (root) => {
    const files = [];
...
const findMissingTranslations = (lang) => {
    const localeFile = Path.resolve(__dirname, "..", "i18n", "locales", lang + ".json");
...
const main = () => {
    ...
    const directoriesToScan = [
        ...
        Path.resolve(__dirname, "..", "control"),
    ...
    for (const dir of directoriesToScan) {
        scanned.push(...scanDirectories(dir));
    }

    scanned.push(Path.resolve(__dirname, "..", "error.vue"));
    ...
    for (const file of translationFiles) {
        ...
        const missingTranslationsFile = Path.resolve(__dirname, "..", "i18n", "locales", lang + ".missing.txt");
