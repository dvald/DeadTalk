...
function generateAPIBindings(basePath, requestLibrary, engine) {
    ...
    const replaceOrRemoveLine = (lines, placeholder, value) => {
        const index = lines.indexOf(placeholder);
        if (index < 0) {
            return;
        }
        if (value) {
    ...
    for (let group of apiGroups) {
        ...
        const usedLibraryValueImports = ["RequestErrorHandler"];
        const usedLibraryTypeImports = ["RequestParams"];
        ...
        const lines = [];
        const LIB_IMPORT_PLACEHOLDER = "__IMPORT_LIBRARY__";
        const LIB_TYPE_IMPORT_PLACEHOLDER = "__IMPORT_LIBRARY_TYPE__";
        const UTILS_IMPORT_PLACEHOLDER = "__IMPORT_UTILS__";
        const DEFINITIONS_IMPORT_PLACEHOLDER = "__IMPORT_DEFINITIONS__";

        lines.push('// API bindings: ' + group + " (Auto generated)");
        lines.push('');
        lines.push('"use strict";');
        lines.push('');
        lines.push(LIB_IMPORT_PLACEHOLDER);
        lines.push(LIB_TYPE_IMPORT_PLACEHOLDER);
        lines.push(UTILS_IMPORT_PLACEHOLDER);
        lines.push(DEFINITIONS_IMPORT_PLACEHOLDER);
        lines.push('');
        ...
        for (let path of Object.keys(paths)) {
            for (let method of Object.keys(paths[path])) {
                ...
                if (apiDef.security && apiDef.security.length > 0) {
                    ...
                    addUnique(usedLibraryTypeImports, "CommonAuthenticatedErrorHandler");
                    ...
                    addUnique(usedLibraryTypeImports, "CommonErrorHandler");
                }
                ...
                if (formDataParameters.length > 0) {
                    ...
                    formDataParameters.forEach(p => {
                        if (engine === "mobile") {
                            if (p.required) {
                                lines.push(tab + tab + 'appendFileToForm(formData, ' + JSON.stringify(p.name) + ", " + "formParams." + p.name + ");");
                            } else {
                                lines.push(tab + tab + "if (formParams." + p.name + " !== undefined) {");
                                lines.push(tab + tab + tab + 'appendFileToForm(formData, ' + JSON.stringify(p.name) + ", " + "formParams." + p.name + ");");
                                lines.push(tab + tab + "}");
                            }
                        } else {
                            if (p.required) {
                                lines.push(tab + tab + 'formData.append(' + JSON.stringify(p.name) + ", " + "formParams." + p.name + ");");
                            } else {
                                lines.push(tab + tab + "if (formParams." + p.name + " !== undefined) {");
                                lines.push(tab + tab + tab + 'formData.append(' + JSON.stringify(p.name) + ", " + "formParams." + p.name + ");");
                                lines.push(tab + tab + "}");
                            }
                        }
                    });
        ...
        replaceOrRemoveLine(lines, LIB_IMPORT_PLACEHOLDER, 'import { ' + usedLibraryValueImports.join(", ") + ' } from "' + requestLibrary + '";');
        replaceOrRemoveLine(lines, LIB_TYPE_IMPORT_PLACEHOLDER, 'import type { ' + usedLibraryTypeImports.join(", ") + ' } from "' + requestLibrary + '";');
        replaceOrRemoveLine(lines, UTILS_IMPORT_PLACEHOLDER, 'import { ' + usedUtils.join(", ") + ' } from "./utils";');
        if (usedDefinitions.length > 0) {
            replaceOrRemoveLine(lines, DEFINITIONS_IMPORT_PLACEHOLDER, 'import type { ' + usedDefinitions.join(", ") + ' } from "./definitions";');
        } else {
            replaceOrRemoveLine(lines, DEFINITIONS_IMPORT_PLACEHOLDER, "");
        }
...
function removeOldApiBindings(basePath, preservedNames) {
...
function main() {
    ...
    const preservedApiBindingEntries = ["utils", "utils.ts"];
    const backendApiBindingsPath = Path.resolve(__dirname, "..", "test", "mocha", "test-tools", "api-bindings");
    const frontendApiBindingsPath = Path.resolve(__dirname, "..", "..", "frontend", "api");
    const mobileApiBasePath = Path.resolve(__dirname, "..", "..", "..", "mobile-application", "src", "api");

    removeOldApiBindings(backendApiBindingsPath, preservedApiBindingEntries);
    removeOldApiBindings(frontendApiBindingsPath, preservedApiBindingEntries);
    
    generateDefinitions(Path.resolve(backendApiBindingsPath, "definitions.ts"));
    generateAPIBindings(backendApiBindingsPath, "@asanrom/request-axios", "node");

    generateDefinitions(Path.resolve(frontendApiBindingsPath, "definitions.ts"));
    generateAPIBindings(frontendApiBindingsPath, "@asanrom/request-browser", "browser");

    if (FS.existsSync(mobileApiBasePath)) {
        removeOldApiBindings(mobileApiBasePath, preservedApiBindingEntries);
        generateDefinitions(Path.resolve(mobileApiBasePath, "definitions.ts"));
        generateAPIBindings(mobileApiBasePath, "@asanrom/request-browser", "mobile");
    } else {
        console.log("Skipped mobile API bindings generation: path not found -> " + mobileApiBasePath);
    }
}
