...
describe("API / Admin / Users", () => {
    ...
    it('Should fail if called by unauthorized user', async () => {
        ...
        await APITester.TestError(ApiUsersAdmin.ChangeUsernameFromAdmin('test', { username: 'test' }), testUser.auth, 403);
        await APITester.TestError(ApiUsersAdmin.ChangeEmailFromAdmin('test', { email: 'test@example.com' }), testUser.auth, 403);
        await APITester.TestError(ApiUsersAdmin.ChangePasswordFromAdmin('test', { password: 'password123456' }), testUser.auth, 403);
    });
    ...
    it('Should be able to change username (Management)', async () => {
        await APITester.Test(ApiUsersAdmin.ChangeUsernameFromAdmin(testUser.uid, { username: newUsername }), testAdmin.auth);
    ...
    it('Should be able to change email (Management)', async () => {
        await APITester.Test(ApiUsersAdmin.ChangeEmailFromAdmin(testUser.uid, { email: newEmail }), testAdmin.auth);
    ...
    it('Should be able to change password (Management)', async () => {
        ...
        await APITester.Test(ApiUsersAdmin.ChangePasswordFromAdmin(testUser.uid, { password: newPassword }), testAdmin.auth);
